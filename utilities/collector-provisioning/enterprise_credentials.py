#!/usr/bin/env python3
"""
Redis Enterprise Collector Permissions Management Script
This script can create new collectors or update existing permissions
...
class RedisEnterpriseAPI:
    ...
    def __init__(
        self, endpoint: str, username: str, password: str, skip_ssl_verify: bool = True
    ):
    ...
    def create_user(
        self, email: str, password: str, name: str, role_uids: List[int]
    ) -> Dict:
        """Create a new user"""
        data = {
            "email": email,
    ...
    def update_database_permissions(
        self, db_uid: int, roles_permissions: List[Dict]
    ) -> bool:
...
class AgentManager:
    """Manages Redis Enterprise collector creation and permission updates"""

    def __init__(self, api: RedisEnterpriseAPI):
        self.api = api

    def find_existing_agent(self, agent_name: str) -> Optional[Dict]:
        """Find existing collector components"""
        existing = {}
        ...
        try:
            ...
            for user in users:
                ...
                # Also check if the name or email contains the collector name
                user_name = user.get("name", "")
                user_email = user.get("email", "")

                if (
                    user_name == agent_name
                    or user_email == f"{agent_name}@example.com"
                    or user_email == agent_name
                    or user_name == f"{agent_name}@example.com"
                    or user_name == f"{agent_name}@re.demo"
                    or user_email == f"{agent_name}@re.demo"
                ):
                    existing["user"] = user
                    break
        except requests.RequestException:
    ...
    def wait_for_component_deletion(
        self,
        component_type: str,
        component_name: str,
        max_retries: int = 10,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(
                        f"    Waiting for {component_type} deletion to propagate... (attempt {attempt + 1}/{max_retries})"
                    )
                    time.sleep(delay)
            except requests.RequestException:
                if attempt < max_retries - 1:
    ...
    def create_new_agent(
        ...
        agent_password: str,
        acl_rules: str = None,
        role_management: str = "cluster_member",
        agent_email: str = None,
        database_filter: str = None,
        skip_existing: bool = False,
        force: bool = False,
        skip_all_databases: bool = False,
        skip_user_creation: bool = False,
    ) -> bool:
        """Create a new collector with ACL, role, user, and database permissions"""
        ...
        try:
            ...
            if existing_components:
                ...
                if missing_components:
                    ...
                    for component, details in existing_components.items():
                        print(
                            f"  ✓ {component.upper()}: {details['name']} (UID: {details['uid']})"
                        )
                    ...
                    if not force:
                        ...
                        print(
                            "Use --force to recreate missing components or --update to update existing permissions."
                        )
                        return False
                    else:
                        print(
                    ...
                    if not force:
                        ...
                        if (
                            "role" in existing_components
                            and "acl" in existing_components
                        ):
                            role_uid = existing_components["role"]["uid"]
                            acl_uid = existing_components["acl"]["uid"]
                            print(
                                f"  Cleaning up database permissions for role UID {role_uid} and ACL UID {acl_uid}"
                            )
                            if not self.cleanup_database_permissions(
                                role_uid, acl_uid, agent_name, database_filter
                            ):
                                print(
                                    f"    ⚠ Warning: Database permissions cleanup may have failed"
                                )
                        ...
                        for component in deletion_order:
                            if component in existing_components:
                                ...
                                print(
                                    f"  Deleting {component}: {details['name']} (UID: {details['uid']})"
                                )
                                try:
                                    if component == "acl":
                                    ...
                                    print(
                                        f"    ✓ {component.upper()} deleted successfully"
                                    )

                                    # Wait for deletion to propagate
                                    if not self.wait_for_component_deletion(
                                        component, details["name"]
                                    ):
                                        print(
                                            f"    ⚠ Warning: {component} deletion may not have propagated fully"
                                        )
                                except requests.RequestException as e:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if "409" in str(e) and "Conflict" in str(e):
                            if attempt < max_retries - 1:
                                print(
                                    f"    ACL still exists, waiting before retry... (attempt {attempt + 1}/{max_retries})"
                                )
                                time.sleep(3.0)
                            else:
                                # Final attempt - check if ACL actually exists and try to get its UID
                                print(
                                    f"    Checking if ACL '{acl_name}' actually exists..."
                                )
                                try:
                                    acls = self.api.get_acls()
                                    existing_acl = next(
                                        (a for a in acls if a["name"] == acl_name), None
                                    )
                                    if existing_acl:
                                        print(
                                            f"    ✓ Found existing ACL with UID: {existing_acl['uid']}"
                                        )
                                        acl = existing_acl
                                        break
                                    else:
                                        print(
                                            f"⚠ ACL '{acl_name}' still exists after deletion. Use --force to recreate."
                                        )
                                        return False
                                except requests.RequestException:
                                    print(
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if "409" in str(e) and "Conflict" in str(e):
                            if attempt < max_retries - 1:
                                print(
                                    f"    Role still exists, waiting before retry... (attempt {attempt + 1}/{max_retries})"
                                )
                                time.sleep(3.0)
                            else:
                                # Final attempt - check if role actually exists and try to get its UID
                                print(
                                    f"    Checking if role '{role_name}' actually exists..."
                                )
                                try:
                                    roles = self.api.get_roles()
                                    existing_role = next(
                                        (r for r in roles if r["name"] == role_name),
                                        None,
                                    )
                                    if existing_role:
                                        print(
                                            f"    ✓ Found existing role with UID: {existing_role['uid']}"
                                        )
                                        role = existing_role
                                        break
                                    else:
                                        print(
                                            f"⚠ Role '{role_name}' still exists after deletion. Use --force to recreate."
                                        )
                                        return False
                                except requests.RequestException:
                                    print(
                    ...
                    for attempt in range(max_retries):
                        try:
                            user = self.api.create_user(
                                agent_email, agent_password, agent_name, [role["uid"]]
                            )
                            print(
                                f"✓ User created successfully with UID: {user['uid']}"
                            )
                            break
                        except requests.RequestException as e:
                            if "409" in str(e) and "Conflict" in str(e):
                                if attempt < max_retries - 1:
                                    print(
                                        f"    User still exists, waiting before retry... (attempt {attempt + 1}/{max_retries})"
                                    )
                                    time.sleep(3.0)
                                else:
                                    # Final attempt - check if user actually exists and try to get its UID
                                    print(
                                        f"    Checking if user '{agent_name}' actually exists..."
                                    )
                                    try:
                                        users = self.api.get_users()
                                        existing_user = next(
                                            (
                                                u
                                                for u in users
                                                if u["name"] == agent_name
                                            ),
                                            None,
                                        )
                                        if existing_user:
                                            print(
                                                f"    ✓ Found existing user with UID: {existing_user['uid']}"
                                            )
                                            user = existing_user
                                            break
                                        else:
                                            print(
                                                f"⚠ User '{agent_name}' still exists after deletion. Use --force to recreate."
                                            )
                                            return False
                                    except requests.RequestException:
                                        print(
            ...
            if not skip_all_databases:
                self.update_database_permissions(
                    role["uid"], acl["uid"], agent_name, database_filter, skip_existing
                )
            else:
    ...
    def update_existing_agent(
        self, agent_name: str, database_filter: str = None, skip_existing: bool = False
    ) -> bool:
        ...
        try:
            ...
            if not acl:
                print(
                    f"✗ Could not find ACL for agent '{agent_name}': {agent_name}-acl"
                )
                return False

            if not role:
                print(
                    f"✗ Could not find role for agent '{agent_name}': {agent_name}-role"
                )
                return False
    ...
    def repair_missing_components(
        ...
        """Repair missing components for an existing collector"""
        ...
        try:
            ...
            if not existing_components:
                print(
                    f"✗ No components found for agent '{agent_name}'. Use create_new_agent instead."
                )
                return False
            ...
            if not missing_components:
                print(
                    f"✓ All components for agent '{agent_name}' already exist. No repair needed."
                )
                return True

            print(f"Missing components: {', '.join(missing_components)}")

            # Create missing components using the same logic as create_new_collector
                    ...
                    try:
                        ...
                        if "409" in str(e) and "Conflict" in str(e):
                            if attempt < max_retries - 1:
                                ...
                                print(
                                    f"⚠ ACL '{acl_name}' still exists. Skipping creation."
                                )
                                break
                        else:
                            raise
                    ...
                    try:
                        ...
                        if "409" in str(e) and "Conflict" in str(e):
                            if attempt < max_retries - 1:
                                ...
                                print(
                                    f"⚠ Role '{role_name}' still exists. Skipping creation."
                                )
                                break
                        else:
                            raise
                    ...
                    try:
                        ...
                        if "409" in str(e) and "Conflict" in str(e):
                            if attempt < max_retries - 1:
                                ...
                                print(
                                    f"⚠ User '{agent_name}' still exists. Skipping creation."
                                )
                                break
                        else:
                            raise
            ...
            if acl and role and not skip_all_databases:
                self.update_database_permissions(
                    role["uid"], acl["uid"], agent_name, database_filter, False
                )
            else:
                print(
                    "Skipping database permissions (missing ACL/role or --skip-all-databases flag set)"
                )
    ...
    def cleanup_database_permissions(
        self, role_uid: int, acl_uid: int, agent_name: str, database_filter: str = None
    ) -> bool:
        ...
        try:
            ...
            if database_filter:
                try:
                    ...
                    filtered_databases = [
                        db for db in databases if pattern.search(db["name"])
                    ]
                    print(
                        f"Filtering databases with pattern '{database_filter}': {len(filtered_databases)}/{len(databases)} databases match"
                    )
            ...
            for db in databases:
                ...
                updated_permissions = [
                    ...
                    if not (
                        perm.get("role_uid") == role_uid
                        and perm.get("redis_acl_uid") == acl_uid
                    )
                ]

                if len(updated_permissions) < len(current_permissions):
                    # Permissions were removed, update the database
                    if self.api.update_database_permissions(
                        db_uid, updated_permissions
                    ):
                        removed_count = len(current_permissions) - len(
                            updated_permissions
                        )
        ...
        try:
            ...
            for db in databases:
                ...
                if permission_exists:
                    ...
                    print(
                        f"    Debug: Adding permission role_uid={role_uid}, acl_uid={acl_uid}"
                    )
                    print(
                        f"    Debug: Current permissions count: {len(current_permissions)}, Updated permissions count: {len(updated_permissions)}"
                    )
                    ...
                    if self.api.update_database_permissions(
                        ...
                        print(
                            f"  ✓ {db_name}: Permission {'set' if not current_permissions else 'augmented'}"
                        )
                        success_count += 1
                    else:
...
def extract_rest_api_details(deployment: Dict) -> Optional[Dict]:
    ...
    if "host" in rest_api and "port" in rest_api:
        ...
        if not validate_host(host):
            print(
                f"✗ Invalid host format in deployment {deployment.get('id', 'unknown')}: '{host}'"
            )
        ...
        if not redis_urls:
            print(
                f"⚠ No redis_urls found for deployment {deployment.get('id', 'unknown')}"
            )
            return None
    ...
    if basic_auth:
        try:
            ...
            print(
                f"⚠ Invalid basic_auth format in deployment {deployment.get('id', 'unknown')}"
            )
            return None
    else:
        username = None
...
def provision_single_cluster(
    ...
    # Check if collector permissions exist
...
def handle_single_cluster_interactive(args, last_values: Dict) -> bool:
    ...
    if args.endpoint:
        ...
        endpoint = prompt_with_default(
            "Enter Redis Enterprise REST API endpoint",
            last_values.get("endpoint", "https://localhost:9443"),
        )
    ...
    if args.username:
        ...
        username = prompt_with_default(
            "Enter admin username for provisioning",
            last_values.get("username", "admin@example.com"),
        )
    ...
    if args.agent_name:
        ...
        agent_name = prompt_with_default(
            "Enter agent name for permissions",
            last_values.get("agent_name", "radar-agent"),
        )

    # Get collector password
    ...
    # Get collector email (user)
    if args.agent_email:
        agent_email = args.agent_email
    elif os.getenv("AGENT_USER"):
        agent_email = os.getenv("AGENT_USER")
    else:
        # Prompt for collector email/username in interactive mode
        default_email = f"{agent_name}@example.com"
        agent_email = prompt_with_default(
            "Enter agent email/username", last_values.get("agent_email", default_email)
        )
    ...
    if existing_agent and not (args.create and args.force):
        ...
        if args.update:
            ...
            success = manager.update_existing_agent(
                agent_name, args.database_filter, args.skip_existing
            )
        elif args.repair:
            ...
            success = manager.repair_missing_components(
                ...
                agent_email,
                args.database_filter,
                args.skip_all_databases,
                False,
            )
            ...
            print("1) Update permissions for existing collector")
            print("2) Create permissions for a new collector with a different name")
            print("3) Repair missing components (create only missing ACL, role, user)")
            print(
                "4) Force recreate existing components (delete and recreate ACL, role, user)"
            )
            print("5) Exit")
            ...
            while True:
                try:
                    ...
                    if choice == "1":
                        ...
                        new_name = prompt_with_default(
                            "Enter new agent name", f"{agent_name}-new"
                        )
                        new_password = prompt_with_default("Enter agent password")
                        new_email = prompt_with_default(
                            "Enter agent email/username", f"{new_name}@example.com"
                        )
                        success = manager.create_new_agent(
                            new_name,
                            new_password,
        ...
        # Collector permissions don't exist or force create mode
        print(
...
def handle_multi_cluster_interactive(args, last_values: Dict) -> bool:
    ...
    config_path = prompt_with_default(
        "Enter path to agent YAML config file", last_values.get("config_path", "")
    )
    if not config_path:
...
def provision_from_yaml_config(
    config_path: str,
    agent_name: str,
    acl_rules: str,
    role_management: str,
    ...
    for i, config in enumerate(cluster_configs, 1):
        ...
        username = config["username"] or prompt_with_default(
            f"Enter admin username for provisioning {config['deployment_name']}",
            "admin@example.com",
        )
        password = config["password"] or prompt_with_default(
            f"Enter admin password for provisioning {config['deployment_name']}"
        )

        # Determine collector credentials based on basic auth presence
        if config["username"] and config["password"]:
            # Use basic auth credentials as collector credentials, skip user creation
            ...
            # Use provided collector credentials and create user
            # Determine collector name if not provided
            if not agent_name:
                if hasattr(args, "agent_name") and args.agent_name:
                    ...
                    agent_name = prompt_with_default(
                        "Enter agent name for permissions", "radar-agent"
                    )

            # Use collector name for both the username and the permissions
            agent_username = agent_name
            agent_password = prompt_with_default(
                f"Enter agent password for {config['deployment_name']}"
            )
            ...
            print(
                "   To avoid password prompts in future runs, add basic_auth to your YAML config:"
            )
            print(f"   deployment:")
            ...
            print(f"       rest_api:")
            print(
                f"         host: \"{config['endpoint'].replace('https://', '').split(':')[0]}\""
            )
    ...
    print(f"\n{'='*60}")
    print(
        f"Multi-cluster provisioning completed: {success_count}/{total_count} clusters successful"
    )
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Redis Enterprise Collector Permissions Manager - Create and manage permissions for monitoring collectors",
        epilog="""
EXAMPLES:
  # Interactive mode - choose between single and multi-cluster provisioning
  python3 enterprise_credentials.py

  # Create permissions for new collector with command line arguments
  ...
  # Provision multiple clusters from collector YAML config
  ...
  # Repair missing components for existing collector
  ...
  python3 enterprise_credentials.py --agent-name radar-agent \\
    ...
    connection_group.add_argument(
        "--username", help="Admin username (e.g., admin@re.demo)"
    )
    connection_group.add_argument("--password", help="Admin password")
    connection_group.add_argument(
        "--verify-ssl",
        action="store_true",
    ...
    # Collector arguments
    agent_group = parser.add_argument_group("Collector Options")
    agent_group.add_argument(
        "--agent-yaml-config",
        help="Path to collector YAML config file (will provision for all ENTERPRISE deployments)",
    )
    agent_group.add_argument(
        "--agent-name",
        help="Collector name (will create collector-name-acl and collector-name-role permissions)",
    )
    agent_group.add_argument(
        "--agent-password", help="Collector password (will prompt if not provided)"
    )
    agent_group.add_argument(
        "--agent-email", help="Collector email (default: collector-name@example.com)"
    )
    ...
    acl_group.add_argument(
        ...
        help="ACL rules for the collector (default: monitoring permissions)",
    )
    ...
    action_group.add_argument(
        "--create",
        action="store_true",
        help="Force create new collector permissions (skip interactive prompts)",
    )
    action_group.add_argument(
        "--update",
        action="store_true",
        help="Force update existing collector permissions",
    )
    action_group.add_argument(
        "--repair",
        action="store_true",
        help="Repair missing components for existing collector (create only missing ACL, role, user)",
    )
    action_group.add_argument(
        "--force",
        action="store_true",
    ...
    db_group.add_argument(
        "--database-filter", help="Only update databases matching this pattern (regex)"
    )
    db_group.add_argument(
        "--skip-existing",
        action="store_true",
    ...
    if args.create or args.update or args.repair:
        ...
        # Get collector password if not provided
        agent_password = args.agent_password
        ...
        # Get collector email if not provided
        agent_email = args.agent_email
        if not agent_email:
        ...
        try:
            ...
            api = RedisEnterpriseAPI(
                args.endpoint, args.username, args.password, skip_ssl_verify
            )
        except Exception as e:
            print(f"✗ Error initializing API client: {e}")
            sys.exit(1)
    ...
    while True:
        print("=" * 50)
        print("Redis Enterprise Collector Permissions Manager")
        print("=" * 50)
        print()
        ...
        try:
            again = (
                input("\nWould you like to provision another cluster? (y/n): ")
                .strip()
                .lower()
            )
            if again != "y":
